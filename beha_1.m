...
%q=1;
   %for q=51:100                              
    ...
    %gen.tot_ntrials = gen.tot_ntrials + 1;
     gen.itr_trial_inv(1,q)= 10;% time to chk for nose poke 
    flag2=0;
     beha_nosepokechk(gen.itr_trial_inv(1,q));
     if (flag2==1)
...
%         continue;
    ...
    % for mouse to coem out water geting water 
%end
%% final save of mat file generated 

 clc;
gen.hitper= length(find(gen.resp == 1))/((length(find(gen.resp == 1)))+(length(find(gen.resp == 4))))
gen.faper= length(find(gen.resp == 2))/((length(find(gen.resp == 2)))+(length(find(gen.resp == 3))))
gen.perfrate= (length(find(gen.resp == 1))+length(find(gen.resp == 3)))/((length(find(gen.resp == 1)))+(length(find(gen.resp == 2)))+(length(find(gen.resp == 3)))+(length(find(gen.resp == 4))));
